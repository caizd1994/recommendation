...
class FMModel(object):
  ...
  def fm_model_fn(features, labels, mode, params):
    ...
    ##second order
    ### feature * embeddings
    print("feature_values",feature_values.get_shape().as_list())
    print("embeddings",embeddings.get_shape().as_list())
    # print(feature_values.eval())
    f_e_m = tf.multiply(feature_values, embeddings)
    print("f_e_m",f_e_m.get_shape().as_list())
    ###  square(sum(feature * embedding))
    f_e_m_sum = tf.reduce_sum(f_e_m, 1)
    print("f_e_m_sum", f_e_m_sum.get_shape().as_list())
    f_e_m_sum_square = tf.square(f_e_m_sum)
    print("f_e_m_sum_square", f_e_m_sum_square.get_shape().as_list())
    ...
    print(tf.metrics.auc(labels,predicts))
